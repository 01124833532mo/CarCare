...
namespace CarCare.Core.Application.Services.ServiceRequests
{
		...
		: IRequestService
	{
		...
		public async Task<ReturnRequestDto> CreateRequestAutomatic(CreateRequestDto requestDto)
		{
			...
			request.JopId = BackgroundJob.Schedule(() => CheckStatus(returnedData.Id), TimeSpan.FromSeconds(10));
		...
		{
			var result = await ReceivedRequest(requestId, Status.InProgress);
			return result;
		}
		...
		public async Task<string> RejectRequest(int requestId)
		{
			...
			if (request.IsAutomatic)
				result = await ReceivedRequest(requestId, Status.Canceled);
		...
		public async Task<string> CompleteRequest(int requestId)
		{
			...
			var result = await ReceivedRequest(requestId, Status.Completed);
		...
		public async Task<IEnumerable<ReturnRequestDto>> GetAllPendingRequestsToTechnical(ClaimsPrincipal claimsPrincipal)
		{
			...
			var userRequests = requests.Where(r => r.TechId == techId && (r.Status != Status.Completed && r.Status != Status.Canceled));
