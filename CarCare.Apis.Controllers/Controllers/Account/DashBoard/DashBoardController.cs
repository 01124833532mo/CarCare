...
namespace CarCare.Apis.Controllers.Controllers.Account.DashBoard
{
	...
	public class DashBoardController(IServiceManager serviceManager) : BaseApiController
	{
		...
		public async Task<ActionResult<IEnumerable<ReturnRequestDto>>> GetRequests([FromQuery] string userId, [FromQuery] int pageSize, [FromQuery] int pageIndex)
		{
			var result = await serviceManager.RequestService.GetAllRequeststoUserForAdmin(userId, pageSize, pageIndex);
			return Ok(result);
		}
